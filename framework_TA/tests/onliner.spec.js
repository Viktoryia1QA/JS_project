...
test.describe('Check onliner.by', () => {
  ...
  test(`25. Check sorting on Item Catalog page `, async ({ page }) => {
    const I = new PageFactory(page);
    await page.goto('https://catalog.onliner.by/mobile/apple/iphone13/prices?order=price%3Aasc');
    const sortingList = await (await I.catalogItemPage.getSortingOnItemCatalogPage());
    const firstItemOnPage = await I.catalogItemPage.listOfPriceByNumber(0).textContent();
    await expect(sortingList[0]).toBe(firstItemOnPage);
  })
